class Translation(object):
    START_TEXT = """Hi {} 👋

I'm Advance Url Uploader Bot 🚀

<b>Permanent Thumbnail Support💯.</b>

<i>sᴇɴᴅ ᴍᴇ ᴀ ᴅɪʀᴇᴄᴛ ʟɪɴᴋ ᴀɴᴅ ɪ ᴡɪʟʟ ᴜᴘʟᴏᴀᴅ ɪᴛ ᴛᴏ ᴛᴇʟᴇɢʀᴀᴍ ᴀs ᴀ ғɪʟᴇ/ᴠɪᴅᴇᴏ.</i>

Click /help for more details...."""
    ADD_CAPTION_HELP = """sᴇʟᴇᴄᴛ ᴀɴ ᴜᴘʟᴏᴀᴅᴇᴅ ғɪʟᴇ/ᴠɪᴅᴇᴏ ᴏʀ ғᴏʀᴡᴀʀᴅ ᴍᴇ <b> ᴀɴʏ ᴛᴇʟᴇɢʀᴀᴍ ғɪʟᴇ</b> ᴀɴᴅ ᴊᴜsᴛ ᴡʀɪᴛᴇ ᴛʜᴇ ᴛᴇxᴛ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ʙᴇ ᴏɴ ᴛʜᴇ ғɪʟᴇ <b>ᴀs ᴀ ʀᴇᴘʟʏ ᴛᴏ ᴛʜᴇ ғɪʟᴇ</b>ᴀɴᴅ ᴛʜᴇ ᴛᴇxᴛ ʏᴏᴜ ᴡʀᴏᴛᴇ ᴡɪʟʟ ʙᴇ ᴀᴛᴛᴀᴄʜᴇᴅ ᴀs ᴛʜᴇ ᴄᴀᴘᴛɪᴏɴ! 🤩
    
Example: <a href='https://te.legra.ph/file/75e0a02003b5378b55e21.jpg'>See This!</a> 👇"""

    INCORRECT_REQUEST = """ᴘʟᴇᴀsᴇ ᴍᴀᴋᴇ sᴜʀᴇ ʏᴏᴜ sᴜʙᴍɪᴛ ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ᴄᴏʀʀᴇᴄᴛʟʏ."""
    WAIT_PROCESS_FINISH = """ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ ғᴏʀ ʏᴏᴜʀ ᴄᴜʀʀᴇɴᴛ ғɪʟᴇ ᴛᴏ ғɪɴɪsʜ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ʙᴇғᴏʀᴇ sᴇɴᴅɪɴɢ ᴍᴏʀᴇ ʟɪɴᴋs!

ᴏʀ ᴜsᴇ /ᴄᴀɴᴄᴇʟ ᴛᴏ ᴛᴇʀᴍɪɴᴀᴛᴇ ɪɴᴄᴏᴍᴘʟᴇᴛᴇ ᴘʀᴏᴄᴇssᴇs."""
    PROCESS_CANCELLED = """ғɪʟᴇ ᴜᴘʟᴏᴀᴅ ᴄᴀɴᴄᴇʟʟᴇᴅ
ʏᴏᴜ ᴄᴀɴ ɴᴏᴡ sᴇɴᴅ ᴀ ɴᴇᴡ ᴜʀʟ."""
    NO_PROCESS_FOUND = """🤷‍♂️ ɴᴏ ᴘᴇɴᴅɪɴɢ ᴜᴘʟᴏᴀᴅs ᴡᴇʀᴇ ғᴏᴜɴᴅ. ʏᴏᴜ ᴄᴀɴ ᴜᴘʟᴏᴀᴅ ғɪʟᴇs ʙʏ sᴇɴᴅɪɴɢ ᴀ ʟɪɴᴋ ɴᴏᴡ!"""
    FORMAT_SELECTION = "👇𝗦𝗲𝗹𝗲𝗰𝘁 𝗔𝗻𝗱 𝗖𝗵𝗼𝘀𝗲 𝗬𝗼𝘂𝗿 𝗙𝗼𝗿𝗺𝗮𝘁👇"
    SET_CUSTOM_USERNAME_PASSWORD = """\n👮‍♂ Powered By :</b> @LISA_FAN_LK"""
    DOWNLOAD_START = "📥 DOWNLOADING..."
    UPLOAD_START = "📤 UPLOADING..."
    RCHD_TG_API_LIMIT = "<b>Downloaded in:</b> {} seconds.\n<b>Detected file size:</b> {}.\n\nsᴏʀʀʏ. ʙᴜᴛ, ɪ ᴄᴀɴɴᴏᴛ ᴜᴘʟᴏᴀᴅ ғɪʟᴇs ɢʀᴇᴀᴛᴇʀ ᴛʜᴀɴ 𝟸ɢʙ ᴅᴜᴇ ᴛᴏ ᴛᴇʟᴇɢʀᴀᴍ ᴀᴘɪ ʟɪᴍɪᴛᴀᴛɪᴏɴs 😕."
    #AFTER_SUCCESSFUL_UPLOAD_MSG = "𝘛𝘏𝘈𝘕𝘒𝘚 𝘍𝘖𝘙 𝘜𝘚𝘐𝘕𝘎 𝘔𝘌 🥰"
    AFTER_SUCCESSFUL_UPLOAD_MSG_WITH_TS = "𝘛𝘏𝘈𝘕𝘒𝘚 𝘍𝘖𝘙 𝘜𝘚𝘐𝘕𝘎 𝘔𝘌 🥰\n\n@skmovieslinks"
    SAVED_CUSTOM_THUMB_NAIL = "Save Your Thumbnail ✅."
    DEL_ETED_CUSTOM_THUMB_NAIL = " Delete Your Thumbnail ✅."
    CUSTOM_CAPTION_UL_FILE = "{}"
    NO_VOID_FORMAT_FOUND = """<b>ɪ ᴛʜɪɴᴋ ʏᴏᴜ ʜᴀᴠᴇ ᴇɴᴛᴇʀᴇᴅ ᴀɴ ᴜɴᴀᴄᴄᴇssɪʙʟᴇ ᴜʀʟ ᴏʀ ᴀ ᴘʀɪᴠᴀᴛᴇ ᴜʀʟ.</b>
<i>ɢᴏ ᴄʜᴇᴄᴋ ɪғ ʏᴏᴜ ᴄᴀɴ ᴀᴄᴄᴇss ᴛʜᴇ ᴄᴏɴᴛᴇɴᴛ ɪɴ ᴛʜᴇ ᴜʀʟ ғʀᴏᴍ ʏᴏᴜʀ ʙʀᴏᴡsᴇʀ ғɪʀsᴛ!</i>

<b>YouTubeDL</b> said: {}"""
    HELP_USER = """<b>ʜᴏᴡ ᴛᴏ ᴜsᴇ ᴍᴇ</b> 🤔

1. 🇸‌🇪‌🇳‌🇩‌ 🇾‌🇴‌🇺‌🇷‌ 🇺‌🇷‌🇱‌ 🇱‌🇮‌🇳‌🇰‌

2. 🇸‌🇪‌🇳‌🇩‌ 🇹‌🇭‌🇺‌🇲‌🇧‌🇳‌🇦‌🇮‌🇱‌ 🇵‌🇭‌🇴‌🇹‌🇴‌ 🇹‌🇴‌ 🇸‌🇦‌🇻‌🇪‌ 🇮‌🇹‌ 🇵‌🇪‌🇷‌🇲‌🇦‌🇳‌🇪‌🇳‌🇹‌🇱‌🇾‌.

3. 🇸‌🇪‌🇱‌🇪‌🇨‌🇹‌ 🇹‌🇭‌🇪‌ 🇩‌🇪‌🇸‌🇮‌🇷‌🇪‌🇩‌ 🇴‌🇵‌🇹‌🇮‌🇴‌🇳‌

4. 🇸‌🇪‌🇳‌🇩‌ /🇩‌🇪‌🇱‌🇹‌🇭‌🇺‌🇲‌🇧‌🇳‌🇦‌🇮‌🇱‌ 🇹‌🇴‌ 🇩‌🇪‌🇱‌🇪‌🇹‌🇪‌ 🇾‌🇴‌🇺‌🇷‌ 🇹‌🇭‌🇺‌🇲‌🇧‌🇳‌🇦‌🇮‌🇱‌.

5.  🇺‌🇸‌🇪‌ /🇨‌🇦‌🇵‌🇹‌🇮‌🇴‌🇳‌ 🇹‌🇴‌ 🇸‌🇪‌🇹‌ 🇨‌🇦‌🇵‌🇹‌🇮‌🇴‌🇳‌ 🇦‌🇸‌ 🇷‌🇪‌🇵‌🇱‌🇾‌ 🇹‌🇴‌ 🇲‌🇪‌🇩‌🇮‌🇦‌

6. 🇨‌🇴‌🇲‌🇲‌🇪‌🇳‌🇹‌🇸‌ /🇦‌🇧‌🇴‌🇺‌🇹‌ /🇻‌🇮‌🇪‌🇼‌🇹‌🇭‌🇺‌🇲‌🇧‌🇳‌🇦‌🇮‌🇱‌ /🇮‌🇳‌🇫‌🇴‌
"""
    ABOUT_TEXT = """<b>🔘 My Name :</b> URL Uploader Bot🚀

<b>🔘 Channel :</b> <a href="https://t.me/skmovieslinks">NT BOT</a>

<b>🔘 Source :</b> <a href="https://te.legra.ph/file/2184b7d4cd6fa1cfc8ac4.mp4">ᴄʟɪᴄᴋ ᴍᴀᴛ ᴋᴀʀ ʟᴀᴜᴅᴇ</a>

<b>🔘 Language :</b> <a href="https://www.python.org/">Python 3.10.8</a>

<b>🔘 Framework :</b> <a href="https://docs.pyrogram.org/">Pyrogram 1.4.16</a>


    REPLY_TO_MEDIA_ALBUM_TO_GEN_THUMB = "sᴇɴᴅ ʏᴏᴜʀ ᴛʜᴜᴍʙɴᴀɪʟ ᴘɪᴄ ᴛᴏ ɢᴇɴᴇʀᴀᴛᴇ ᴄᴜsᴛᴏᴍ ᴛʜᴜᴍʙᴀɪʟ."
    ERR_ONLY_TWO_MEDIA_IN_ALBUM = "ᴍᴇᴅɪᴀ ᴀʟʙᴜᴍ sʜᴏᴜʟᴅ ᴄᴏɴᴛᴀɪɴ ᴏɴʟʏ ᴛᴡᴏ ᴘʜᴏᴛᴏs. ᴘʟᴇᴀsᴇ ʀᴇ-sᴇɴᴅ ᴛʜᴇ ᴍᴇᴅɪᴀ ᴀʟʙᴜᴍ, ᴀɴᴅ ᴛʜᴇɴ ᴛʀʏ ᴀɢᴀɪɴ, ᴏʀ sᴇɴᴅ ᴏɴʟʏ ᴛᴡᴏ ᴘʜᴏᴛᴏs ɪɴ ᴀɴ ᴀʟʙᴜᴍ."
    CANCEL_STR = "❌ ℂ𝕒𝕟𝕔𝕖𝕝𝕝𝕖𝕕 ❌"
    ZIP_UPLOADED_STR = "Uploaded {} files in {} seconds."
    SLOW_URL_DECED = "Gosh that seems to be a very slow URL. Since you were screwing my home, I am in no mood to download this file. Meanwhile, why don't you try this:==> https://shrtz.me/PtsVnf6 and get me a fast URL so that I can upload to Telegram, without me slowing down for other users."

    INFO_TEXT = """
🌸 First Name : <b>{}</b>

🌸 Second Name : <b>{}</b>

🌸 Username : <b>@{}</b>

🌸 Id : <code>{}</code>

🌸 Profile : <b>{}</b>

🌸 Dc : <b>{}</b>

🌸 Language : <b>{}</b>

🌸 Status : <b>{}</b>
"""
    # Soon 💯
    #START_BUTTONS = InlineKeyboardMarkup(
       #  [[
       #  InlineKeyboardButton('HELP', callback_data='help')
       #  InlineKeyboardButton('ABOUT', callback_data='about')
       #  ],[
      #   InlineKeyboardButton('CLOSE', callback_data='close')
        # ]]
  #  )
  # BUTTONS = InlineKeyboardMarkup(

